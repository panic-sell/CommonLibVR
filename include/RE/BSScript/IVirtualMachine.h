...
namespace RE
{
	...
	namespace BSScript
	{
		...
		public:
			...
			virtual bool					   FindBoundObject(VMHandle a_handle, const char* a_className, BSTSmartPointer<Object>& a_result) const = 0;																				   // 1E
			virtual void					   MoveBoundObjects(VMHandle a_from, VMHandle a_to) = 0;																																	   // 1F
			virtual void					   ResetAllBoundObjects(VMHandle a_handle) = 0;																																				   // 20
			virtual bool					   CastObject(const BSTSmartPointer<Object>& a_fromObjPtr, const BSTSmartPointer<ObjectTypeInfo>& a_toTypeInfoPtr, BSTSmartPointer<Object>& a_toObjPtr) = 0;								   // 21
			virtual bool					   SetPropertyValue(BSTSmartPointer<Object>& a_obj, const char* a_propertyName, Variable& a_setVal) = 0;																					   // 22
			virtual bool					   GetPropertyValue(BSTSmartPointer<Object>& a_obj, const char* a_propertyName, Variable& a_getVal) = 0;																					   // 23
			virtual bool					   GetVariableValue1(const BSTSmartPointer<Object>& a_objPtr, UInt32 a_index, Variable& a_out) const = 0;																					   // 24
			virtual bool					   GetVariableValue2(VMHandle a_handle, const BSFixedString& a_className, SInt32 a_variableIndex, Variable& a_out) const = 0;																   // 25
			virtual void					   SendEvent(VMHandle a_handle, const BSFixedString& a_eventName, IFunctionArguments* a_args) = 0;																							   // 26
			virtual void					   SendEventAll(const BSFixedString& a_eventName, IFunctionArguments* a_args) = 0;																											   // 27
			virtual bool					   DispatchStaticCall(const BSFixedString& a_className, const BSFixedString& a_fnName, IFunctionArguments* a_args, BSTSmartPointer<IStackCallbackFunctor>& a_result) = 0;					   // 28
			virtual bool					   DispatchMethodCall1(BSTSmartPointer<Object>& a_obj, const BSFixedString& a_fnName, IFunctionArguments* a_args, BSTSmartPointer<IStackCallbackFunctor>& a_result) = 0;					   // 29
			virtual bool					   DispatchMethodCall2(VMHandle a_handle, const BSFixedString& a_className, const BSFixedString& a_fnName, IFunctionArguments* a_args, BSTSmartPointer<IStackCallbackFunctor>& a_result) = 0;  // 2A
			virtual bool					   DispatchUnboundMethodCall(void) = 0;																																						   // 2B
			virtual bool					   IsWaitingOnLatent(VMStackID a_stackID) const = 0;																																		   // 2C
			virtual void					   ReturnFromLatent(VMStackID a_stackID, const Variable& a_val) = 0;																														   // 2D
			virtual ErrorLogger* GetErrorLogger() = 0;																																														// 2E 
			virtual IObjectHandlePolicy* GetObjectHandlePolicy1() = 0;																																										// 2F 
			virtual const IObjectHandlePolicy* GetObjectHandlePolicy2() const = 0;																																						   // 30 
			virtual ObjectBindPolicy* GetObjectBindPolicy1() = 0;																																											// 31 
			virtual const ObjectBindPolicy* GetObjectBindPolicy2() const = 0;																																								// 33 
			virtual ISavePatcherInterface* GetSavePatcherInterface() = 0;																																									// 34 
			virtual void					   RegisterForLogEvent(BSTEventSink<LogEvent>* a_sink) = 0;																																	   // 35
			virtual void					   UnregisterForLogEvent(BSTEventSink<LogEvent>* a_sink) = 0;																																   // 36
			virtual void					   RegisterForStatsEvent(BSTEventSink<StatsEvent>* a_sink) = 0;																																   // 37
			virtual void					   UnregisterForStatsEvent(BSTEventSink<StatsEvent>* a_sink) = 0;																															   // 38
